import fs from 'fs';
import { getIndicatorsStatusInvest } from '../Scrap/indicators-statusinvest.js';
...
if (tickersIndicators.length != tickers.length) {
    ...
    tickers.map(ticker => {
        const temp = tickersIndicators.find(tickerIndicator => ticker.code === tickerIndicator.code);
        if (!temp){
            tickersIndicators.push({code: ticker.code, price: ticker.price, date: ''})
        }
    });
}
...
const tickerIndicatorPendding = tickersIndicators.filter(item => item.date != dataAtual && item.price > 0);

for (const tickerIndicator in tickerIndicatorPendding) {
    const element = tickerIndicatorPendding[tickerIndicator];
    
    const indicators = await getIndicatorsStatusInvest(element.code);

    if(indicators) {

        const objIndex = tickersIndicators.findIndex(item => item.code === element.code) ;

        console.log(`Atualizando ${element.code} ::: ${objIndex} de ${tickersIndicators.length}`);
        
        tickersIndicators[objIndex] = {code: element.code, price: element.price, date: dataAtual, indicators: indicators };
